...
export default function ReprioritizeSuggestions({
  ...
  const handleAccept = async (suggestion: RoadmapReprioritizeSuggestion) => {
    ...
    try {
      ...
      setSuggestions((prev) =>
        prev.filter((item) => item.suggestion_id !== suggestion.suggestion_id),
      );
      onApplied();
    } catch (err: any) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {error && (
        <p className="mt-3 rounded-2xl bg-rose-50 px-4 py-2 text-xs text-rose-600">
          {error}
        </p>
      )}
      {loading ? (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex gap-2 text-xs">
                  ...
                  {suggestion.recommended_order_index !== null &&
                    suggestion.recommended_order_index !== undefined && (
              ...
              <div className="mt-4 flex gap-3">
                ...
                >
                  {applying === suggestion.suggestion_id
                    ? "Applying…"
                    : "Accept"}
                </button>
                <button
